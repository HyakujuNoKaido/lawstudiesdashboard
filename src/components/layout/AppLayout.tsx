...
export function AppLayout() {
  ...
  // NOUVEAU : Gestion des toasts avec Undo
  const [toastData, setToastData] = useState<{ message: string; type: string; id: number; onUndo?: () => void } | null>(null);
  ...
  useEffect(() => {
    const handleToast = (e: Event) => {
      ...
      // On laisse plus de temps si un bouton Annuler est présent (6 secondes au lieu de 3)
      setTimeout(() => {
        setToastData(current => current?.id === newToast.id ? null : current);
      }, customEvent.detail.onUndo ? 6000 : 3000);
    };
  ...
  return (
    <div className={`min-h-screen bg-background text-text flex flex-col antialiased selection:bg-accent/30 selection:text-accent relative overflow-x-hidden ${localFocus ? 'grayscale-[0.2] contrast-125' : ''}`}>
                  ...
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-3 w-64 bg-surface-elevated border border-border rounded-modal shadow-apple z-50 py-2 animate-in fade-in slide-in-from-top-2">
                      <div className="px-4 py-3 border-b border-border/50 mb-2">
                        <p className="font-bold text-sm text-text truncate">{user?.email || 'Mode Solo'}</p>
                      ...
                      {user ? (
                        ...
                        <button onClick={() => { setIsAvatarMenuOpen(false); navigate('/onboarding'); }} className="w-full px-4 py-2.5 text-sm text-accent hover:bg-accent/10 flex items-center gap-3 cursor-pointer transition-colors"><User size={16} /> Créer un compte</button>
                      )}
                    </div>
                  </>
                )}
      ...
      {/* SYSTÈME DE TOASTS ENRICHI (Avec Undo) */}
      {toastData && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-5 fade-in duration-300 pointer-events-none">
          <div className={`flex items-center gap-2.5 px-4 py-3 rounded-card shadow-apple-subtle border backdrop-blur-xl text-sm font-medium pointer-events-auto
            ${toastData.type === 'success' ? 'bg-success/15 border-success/30 text-success' : ''}
            ${toastData.type === 'error' ? 'bg-danger/15 border-danger/30 text-danger' : ''}
            ${toastData.type === 'info' ? 'bg-info/15 border-info/30 text-info' : ''}
            ${toastData.type === 'warning' ? 'bg-warning/15 border-warning/30 text-warning' : ''}
          `}>
            {toastData.type === 'success' && <LexiIcons.Success size={18} />}
            {toastData.type === 'error' && <LexiIcons.Error size={18} />}
            {toastData.type === 'info' && <LexiIcons.Info size={18} />}
            {toastData.type === 'warning' && <LexiIcons.Warning size={18} />}
            ...
            {toastData.onUndo && (
              <>
                ...
                <button 
                  onClick={() => { toastData.onUndo!(); setToastData(null); }}
                  className="text-xs font-bold hover:text-text transition-colors underline cursor-pointer"
                >
                  Annuler
                </button>
              </>
