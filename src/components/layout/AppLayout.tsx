...
import { Sun, GraduationCap, Library, BrainCircuit, Plus, X, Scale, FileText, Timer, Upload, FileEdit, Search } from 'lucide-react';
...
export function AppLayout() {
  ...
  // Helper pour activer les tabs en fonction des nouvelles routes (qui seront créées aux étapes suivantes)
  ...
  return (
    <div className="min-h-screen bg-background text-text flex flex-col antialiased selection:bg-accent/30 selection:text-accent">
      ...
      {/* Menu d'actions rapides (Action Sheet avec un ton compagnon) */}
      {isActionMenuOpen && (
          ...
          <div className="w-full max-w-md bg-surface border border-border rounded-3xl p-6 shadow-2xl flex flex-col gap-4 animate-in slide-in-from-bottom-5 duration-300">
            ...
            <div className="grid grid-cols-1 gap-2.5">
              <button 
                ...
                className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-surface-elevated border border-border hover:border-warning/50 transition-all text-left group cursor-pointer"
              >
              ...
              <button 
                ...
                className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-surface-elevated border border-border hover:border-info/50 transition-all text-left group cursor-pointer"
              >
              ...
              <button 
                ...
                className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-surface-elevated border border-border hover:border-danger/50 transition-all text-left group cursor-pointer"
              >
                <div className="w-10 h-10 rounded-xl bg-danger/10 text-danger flex items-center justify-center group-hover:scale-105 transition-transform">
                  <Timer size={20} />
                </div>
                <div>
              ...
              <button 
                ...
                className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-surface-elevated border border-border hover:border-success/50 transition-all text-left group cursor-pointer"
              >
              ...
              <button 
                ...
                className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-surface-elevated border border-border hover:border-accent/50 transition-all text-left group cursor-pointer"
              >
              ...
              <button 
                ...
                className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-surface-elevated border border-border hover:border-secondary/50 transition-all text-left group cursor-pointer"
              >
      ...
      {/* Barre de navigation inférieure (Conceptuelle) */}
      <nav aria-label="Navigation principale" className="fixed bottom-0 left-0 right-0 z-40 bg-surface/90 backdrop-blur-xl border-t border-border px-2 py-2">
        <div className="max-w-md mx-auto flex items-center justify-between relative px-2">
          ...
          <button 
            onClick={() => navigate('/')}
            className={`flex flex-col items-center gap-1.5 p-2 rounded-xl transition-colors cursor-pointer w-16 ${
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1.5 p-2 rounded-xl transition-colors cursor-pointer w-16 ${
              isActive(['/courses', '/schedule']) ? 'text-accent font-semibold' : 'text-text-muted hover:text-text'
            }`}
          >
          ...
          <div className="relative -top-6 mx-2">
            <button 
              ...
              className="w-14 h-14 rounded-2xl bg-accent text-background flex items-center justify-center glow-gold hover:bg-accent-strong transition-transform active:scale-95 shadow-xl cursor-pointer"
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1.5 p-2 rounded-xl transition-colors cursor-pointer w-16 ${
              isActive(['/library']) ? 'text-accent font-semibold' : 'text-text-muted hover:text-text'
            }`}
          >
            <Library size={22} />
