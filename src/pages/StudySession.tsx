  ...
  const handleRating = useCallback(async (quality: number) => {
    ...
    try {
      ...
      if (currentIndex + 1 < flashcards.length) {
        ...
        addLog(`Session terminée : ${flashcards.length} cartes étudiées`, 'study');
      }
    } catch (err) {
  ...
  const handleMatchSelect = (id: string) => {
    ...
    if (first && second && first.refId === second.refId && first.type !== second.type) {
      ...
      if (matchScore + 1 === matchItems.length / 2) {
        setTimeout(() => {
          setSessionCompleted(true);
          addLog(`Jeu d'association réussi (${matchItems.length / 2} paires)`, 'study');
        }, 1000);
      }
    } else {
      setSelectedMatch(null);
      toast("Mauvaise association", "warning");
    }
  };
        ...
        {learningMode === 'test' && (
          <div className="flex flex-col gap-8 max-w-2xl mx-auto w-full animate-in fade-in">
            ...
            <form onSubmit={handleTypeSubmit} className="flex flex-col gap-4">
              <textarea 
                rows={3} autoFocus disabled={feedback !== null} value={typedAnswer} onChange={(e) => setTypedAnswer(e.target.value)} placeholder="Saisissez la réponse..." 
                className={`w-full bg-surface border rounded-card p-5 text-base focus:outline-none transition-colors resize-none shadow-sm ${feedback === 'correct' ? 'border-success bg-success/5 text-success' : feedback === 'close' ? 'border-warning bg-warning/5 text-warning' : feedback === 'wrong' ? 'border-danger bg-danger/5 text-danger' : 'border-border focus:border-accent text-text'}`} 
              />
              {feedback && (
