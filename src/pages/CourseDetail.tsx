...
export function CourseDetail() {
  ...
  const [allCourses, setAllCourses] = useState<any[]>([]); // Pour déplacer les flashcards
  ...
  // Modales d'actions individuelles Flashcards
  const [isCardModalOpen, setIsCardModalOpen] = useState(false);
  const [editingCardId, setEditingCardId] = useState<string | null>(null);
  const [cardForm, setCardForm] = useState({ front: '', back: '' });
  const [cardToDelete, setCardToDelete] = useState<string | null>(null);

  // --- ÉTATS DE SÉLECTION DE MASSE ---
  ...
  const handleBatchDelete = async () => {
    try {
      ...
      // Utilisation du Undo Toast
      toast(`${totalSelectedCount} éléments supprimés`, "success", () => {
        // En vrai système, on restaurerait. Ici on notifie la fonction d'Undo.
        toast("Annulation de la suppression (Simulation)", "info");
        loadData();
      });
  ...
  const openBatchMoveModal = (context: 'chapters_docs' | 'flashcards') => {
  ...
  const handleBatchMoveSubmit = async () => {
    try {
      if (moveContext === 'chapters_docs') {
        await batchMoveItems(selectedChapterIds, selectedDocIds, batchTargetId);
      } else if (moveContext === 'flashcards' && batchTargetId) {
        await batchMoveFlashcards(selectedCardIds, batchTargetId);
      }
      
      toast(`${totalSelectedCount} éléments déplacés`, "success", () => {
        toast("Déplacement annulé (Simulation)", "info");
        loadData();
      });
  ...
  // ... (Garder les autres gestionnaires : CreateChapter, DeleteChapter, IA, etc. identiques)
  ...
  const handleLaunchAI = async () => {
    ...
    } catch (err: any) { toast("L'analyse a échoué.", "error"); }
  };
            ...
            <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
              {!isSelectMode && (
                <>
                  ...
                  <button onClick={() => { setAiTarget({ type: 'chapter', id: chapter.id, name: chapter.title }); setIsAIModalOpen(true); }} className="p-1.5 text-text-muted hover:text-warning transition-colors rounded cursor-pointer group/ai" title="IA"><Sparkles size={16} className="group-hover/ai:animate-pulse" /></button>
                  <div className="pl-1 border-l border-border/50 ml-1">
                    <ResourceMenu onMove={() => { setSelectedChapterIds([chapter.id]); openBatchMoveModal('chapters_docs'); }} onArchive={() => toast("Archivé", "info")} onDelete={() => setChapterToDelete(chapter.id)} />
                  </div>
                </>
              )}
            </div>
    ...
    <div className="flex flex-col gap-6 pt-2 pb-24 animate-in fade-in duration-300">
      <header className="flex flex-col gap-6 text-text">
        <div className="flex items-center justify-between">
          ...
          <ResourceMenu onEdit={() => navigate(`/edit/course/${course.id}`)} onArchive={() => toast("Archivé", "info", () => toast("Restauré", "success"))} onDelete={() => toast("Utilisez la page Plan d'études", "info")} />
        </div>
        <div>
      ...
      <div className="mt-2">
          ...
          <div className="flex flex-col gap-4 animate-in fade-in">
            <div className="flex justify-between items-center mb-2 bg-surface p-4 rounded-2xl border border-border">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setIsSelectMode(!isSelectMode)} className={`text-xs font-bold flex items-center gap-1.5 px-3 py-2 rounded-btn border transition-colors cursor-pointer ${isSelectMode ? 'bg-accent text-background border-accent' : 'bg-surface-elevated border-border text-text hover:bg-surface-interactive'}`}>
                  <CheckSquare size={14} /> Sélection
                </button>
                <button onClick={() => { setParentChapterId(null); setIsAddingChapter(true); }} className="text-xs text-background font-bold flex items-center gap-1 cursor-pointer bg-text px-3 py-2 rounded-btn hover:bg-text/90">
            ...
            {isAddingChapter && (
              <form onSubmit={handleCreateChapter} className="bg-surface border border-accent/40 p-4 rounded-2xl flex flex-col gap-3 animate-in fade-in duration-200">
                <span className="text-xs font-bold text-accent">Nouveau Chapitre</span>
            ...
            {chapters.length === 0 && !isAddingChapter ? (
              <div className="text-center py-16 border border-dashed border-border rounded-2xl text-text-muted text-sm">Aucun chapitre. Construisez la structure du cours.</div>
            ) : (
              <div className="flex flex-col gap-3">
        ...
        {/* ... RESTE DES ONGLETS IDENTIQUES POUR DOCUMENTS, NOTES ET RÉSULTATS ... */}
        
        {/* NOUVEL ONGLET FLASHCARDS AVEC SÉLECTION */}
          ...
          <div className="flex flex-col gap-4 animate-in fade-in">
            <div className="flex justify-between items-center mb-2 bg-surface p-4 rounded-2xl border border-border">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setEditingCardId(null); setCardForm({front: '', back: ''}); setIsCardModalOpen(true); }} className="text-xs bg-surface-elevated border border-border px-3 py-2 rounded-btn font-bold flex items-center gap-1.5 hover:bg-surface-interactive cursor-pointer">
              ...
              <div className="flex flex-col border border-border bg-surface rounded-card overflow-hidden shadow-sm">
                {flashcards.map((card, idx) => {
                  ...
                  return (
                    <div 
                      key={card.id} 
                      onClick={() => isSelectMode ? toggleSelectCard(card.id) : null}
                      className={`p-4 flex items-start gap-4 transition-colors group ${idx !== flashcards.length -1 ? 'border-b border-border/50' : ''} ${isSelected ? 'bg-accent/5 border-l-4 border-l-accent' : 'hover:bg-surface-interactive border-l-4 border-l-transparent'}`}
                    >
                      {isSelectMode && (
                        <div className="mt-1 text-accent cursor-pointer shrink-0">
                          {isSelected ? <CheckSquare size={18} /> : <Square size={18} className="text-text-muted" />}
                        </div>
                      )}
                      ...
                      {!isSelectMode && (
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                          <ResourceMenu 
                            ...
                            onMove={() => { setSelectedCardIds([card.id]); openBatchMoveModal('flashcards'); }}
      ...
      {/* --- BULK TOOLBAR GLOBALE --- */}
      <BulkSelectionToolbar 
        selectedCount={totalSelectedCount}
        onClear={clearSelection}
        onMove={() => {
          if (selectedCardIds.length > 0 && selectedChapterIds.length === 0 && selectedDocIds.length === 0) {
            openBatchMoveModal('flashcards');
          } else if (selectedChapterIds.length > 0 || selectedDocIds.length > 0) {
            openBatchMoveModal('chapters_docs');
          } else {
            toast("Sélection mixte non supportée pour le déplacement", "warning");
          }
        }}
        onArchive={() => toast("Archivé avec succès", "success", () => toast("Archivage annulé", "info"))}
        onDelete={() => setIsBatchDeleteModalOpen(true)}
      />

      {/* MODALE DÉPLACEMENT (Dynamique selon le contexte) */}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={batchTargetId || ''} onChange={(e) => setBatchTargetId(e.target.value ? e.target.value : null)} className="bg-background border border-border rounded-input px-3.5 py-2.5 text-sm text-text w-full focus:border-accent cursor-pointer">
                {moveContext === 'chapters_docs' ? (
                  <>
                    <option value="">(Racine du cours actuel)</option>
                    {chapters.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                  </>
                ) : (
                  <>
                    <option value="" disabled>Sélectionner un Set (Cours)...</option>
      ...
      {/* AUTRES MODALES RESTENT IDENTIQUES */}
