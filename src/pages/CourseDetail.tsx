...
import { ChevronLeft, FileText, Edit3, Trash2, BrainCircuit, FileEdit, Award, LayoutGrid, Scale, Plus, X } from 'lucide-react';
import { Badge } from '../components/ui/Badge';
import { Card } from '../components/ui/Card';
import { ConfirmModal } from '../components/ui/ConfirmModal';
import { fetchCourseById, fetchCourseChapters, fetchCourseDocuments, fetchCourseGrades, fetchNotes, fetchFlashcards, deleteDocument, createChapter } from '../services/supabaseService';
...
export function CourseDetail() {
  ...
  // NOUVEAU : État pour l'ajout de chapitre
  ...
  // NOUVEAU : Fonction de création d'un chapitre
  const handleCreateChapter = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast("Chapitre ajouté avec succès", "success");
      loadData();
    } catch (err) {
        ...
        <section className="lg:col-span-2 flex flex-col gap-4 text-text">
          ...
          {chapters.length === 0 && documents.length === 0 && !isAddingChapter ? (
            <div className="text-center py-12 border border-dashed border-border rounded-2xl text-text-muted text-sm flex flex-col items-center gap-2">
              <p>Ce cours est vide. Commencez par ajouter des chapitres ou importer un plan de cours.</p>
            </div>
          ) : (
