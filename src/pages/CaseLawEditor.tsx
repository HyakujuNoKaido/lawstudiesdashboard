...
import { ChevronLeft, Save, Scale, FileText, CheckCircle2, Sparkles } from 'lucide-react';
    ...
    <div className="flex flex-col gap-6 pt-2 pb-16 animate-in fade-in duration-300 max-w-3xl mx-auto w-full text-text">
      ...
      <header className="flex flex-col gap-4">
        ...
        <div>
          ...
          <p className="text-text-muted text-xs">Synthèse structurée de jurisprudence du Tribunal fédéral.</p>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">{generating ? 'Génération...' : 'Remplir avec l\'IA'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5 bg-surface p-4 rounded-2xl border border-border">
          ...
          <textarea rows={3} required value={form.facts} onChange={(e) => setForm({ ...form, facts: e.target.value })} className="w-full bg-surface-elevated border border-border rounded-xl p-3 text-sm focus:outline-none focus:border-accent font-serif mt-1" />
        </div>
        ...
        <div className="flex flex-col gap-1.5 bg-surface p-4 rounded-2xl border border-border">
          ...
          <textarea rows={3} required value={form.procedure} onChange={(e) => setForm({ ...form, procedure: e.target.value })} className="w-full bg-surface-elevated border border-border rounded-xl p-3 text-sm focus:outline-none focus:border-accent font-serif mt-1" />
        </div>

        <div className="flex flex-col gap-1.5 bg-surface p-4 rounded-2xl border border-border">
          <label className="text-xs font-bold text-accent uppercase tracking-wider flex items-center gap-1.5"><CheckCircle2 size={16} /><span>3. Les Considérants (Erwägungen)</span></label>
          <textarea rows={4} required value={form.consideranda} onChange={(e) => setForm({ ...form, consideranda: e.target.value })} className="w-full bg-surface-elevated border border-border rounded-xl p-3 text-sm focus:outline-none focus:border-accent font-serif mt-1" />
        </div>

        <div className="flex flex-col gap-1.5 bg-surface p-4 rounded-2xl border border-border">
          <label className="text-xs font-bold text-success uppercase tracking-wider flex items-center gap-1.5"><CheckCircle2 size={16} /><span>4. La Solution / Holding</span></label>
          <textarea rows={3} required value={form.holding} onChange={(e) => setForm({ ...form, holding: e.target.value })} className="w-full bg-surface-elevated border border-border rounded-xl p-3 text-sm focus:outline-none focus:border-accent font-serif mt-1" />
        </div>

        <button type="submit" disabled={loading} className="w-full bg-accent text-background rounded-xl py-3.5 px-4 flex items-center justify-center gap-2 font-semibold glow-gold hover:bg-accent-strong transition-colors cursor-pointer">
          <Save size={18} />
