...
export function DocumentUpload() {
  ...
  const [internalName, setInternalName] = useState(''); // Nouveau champ : nom dans l'app
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Pré-remplir le nom interne sans l'extension pour gagner du temps
      if (!internalName) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await uploadCourseDocument(
        ...
        chapterId || undefined, 
        atfRef || undefined,
        internalName || selectedFile.name // On utilise le nom choisi
      );
    ...
    <div className="flex flex-col gap-6 pt-2 pb-6 animate-in fade-in duration-300 max-w-2xl mx-auto w-full text-text">
      <header className="flex flex-col gap-4">
        ...
        <div>
          ...
          <p className="text-text-muted text-sm">Importez un support et donnez-lui un nom clair pour vos révisions.</p>
        </div>
      </header>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          />
          <p className="text-[10px] text-text-muted">Ce nom remplacera le nom du fichier original dans vos listes pour plus de lisibilité.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            ...
            <select 
              ...
              className="w-full bg-surface-elevated border border-border rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-accent transition-colors appearance-none cursor-pointer"
            >
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-text-muted">Chapitre du cours</label>
            <select 
              value={chapterId}
              onChange={(e) => setChapterId(e.target.value)}
              disabled={!courseId}
              className="w-full bg-surface-elevated border border-border rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-accent transition-colors appearance-none cursor-pointer disabled:opacity-50"
            >
